package com.example.cinema.model.content_provider
...
import com.example.cinema.app.App.Companion.getHistoryDao
import com.example.cinema.model.room_data_base.*

private const val URI_ALL = 1
private const val URI_ID = 2
private const val ENTITY_PATH = "HistoryEntity"
class CinemaContentProvider : ContentProvider() {
    ...
    private fun map(values: ContentValues?) : HistoryEntity {
        return if (values == null) {
            ...
            val year = values[YEAR] as Int
            val isLike = values[ISLIKE] as Int

            HistoryEntity(id, name, year = year, isLike = isLike)
        }
    }
}