...
class MovieDTO_repository(context: Context) {
    ...
    fun insertMovieDTO_db(movieDTO_db: MovieDTO_db) {
        insertAsyncTaskMovieDTO_db(db).execute(movieDTO_db)
    }

    fun insertDocs_db(docs_db: Docs_db) {
        insertAsyncTaskDocs_db(db).execute(docs_db)
    }

    fun insertNames_db(names_db: Names_db) {
        insertAsyncTaskNames_db(db).execute(names_db)
    }


    fun updateMovieDTO_db(movieDTO_db: MovieDTO_db) {
        db.updateMovieDTO_db(movieDTO_db)
    }

    fun updateDocs_db(docs_db: Docs_db, index: Int) {
        db.updateDocs_db(docs_db)
    }

    fun updateNames_db(names_db: Names_db, index: Int) {
        db.updateNames_db(names_db)
    }


    fun deleteMovieDTO_db(movieDTO_db: MovieDTO_db, index: Int) {
        db.deleteMovieDTO_db(movieDTO_db)
    }

    fun deleteDocs_db(docs_db: Docs_db, index: Int) {
        db.deleteDocs_db(docs_db)
    }

    fun deleteNames_db(names_db: Names_db, index: Int) {
        db.deleteNames_db(names_db)
    }


    private class insertAsyncTaskMovieDTO_db(private val movieDTO_Dao: MovieDTO_Dao) :
    ...
    private class insertAsyncTaskDocs_db(private val movieDTO_Dao: MovieDTO_Dao) :
    ...
    private class insertAsyncTaskNames_db(private val movieDTO_Dao: MovieDTO_Dao) :
