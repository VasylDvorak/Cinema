...
class DetailsRepositoryImpl(private val remoteDataSource: RemoteDataSource) :
    ...
    override fun getBestMovieDetailsFromServer(find_type: Int, callback: Callback<MovieDTOBest>) {
        remoteDataSource.getBestMovieDetails(find_type, callback) }

    override fun getPlayMovieDetails(idd: Int, callback: Callback<MovieDetails>) {
        remoteDataSource.getPlayMovie(idd, callback) }
